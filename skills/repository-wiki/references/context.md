...
3. Verify every registered source used by the pack is a regular file and matches its manifest identity using the shared path-safety rules. Immediately before returning or saving the pack, recheck path safety, regular-file type, and identity for the index, every selected canonical page, the manifest, and every source actually used. If any maintained page, manifest, or source changed, discard the affected pack and restart from the current consistent state. Report provenance gaps and do not present stale or mixed-version wiki claims as source-supported.
