...
3. Verify every registered source used by the pack is a regular file and matches its manifest identity using the shared path-safety rules, then recheck cited sources, including their regular-file type, immediately before returning or saving the pack. Report provenance gaps and do not present stale wiki claims as source-supported.
