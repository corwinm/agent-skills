...
3. Verify every registered source used by the pack is a regular file and matches its manifest identity using the shared path-safety rules. For discovery-workspace transcript evidence, separately baseline the mutable meeting control manifest without registering it as an immutable source; verify its speaker mapping, discovery-use consent, and direct-quotation consent before using or quoting evidence, and treat `unknown` as not granted. Recheck cited sources and consent-control baselines, including regular-file type, immediately before returning or saving the pack. If consent state changed, reevaluate from the current state. Report provenance gaps and do not present stale or unauthorized wiki claims as source-supported.
...
7. Clearly label uncertainty and omissions caused by budget, missing evidence, consent restrictions, or provenance gaps. Preserve every included interpretation's author and material alternatives; every assumption's owner, consequence if wrong, risk, and proposed validation; and every decision's chosen action, alternatives considered, rationale, evidence used, unresolved dissent, owner, and date. Use `Unknown` for unavailable values.
