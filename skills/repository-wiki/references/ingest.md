...
- For a directory, recursively enumerate relevant documents without following symlinks while excluding `.git`, generated output, dependencies, secrets, and ignored files. For every child directory, inspect components without following links, require a real directory, and verify canonical containment before traversing it. Reject symlinks and special-file leaves; include only regular files. Record the exact included repository-relative source-file set and regular-file types, plus every traversed directory path and directory type, and repeat that non-following membership/type inventory during later requested-directory rechecks.
...
5. Publish staged wiki targets one at a time, leaving the manifest until last, through the shared operation lock and compare-and-swap publication procedure. Never stream into or truncate a canonical path. For each target, prepare and verify the complete same-directory temporary regular file, then repeat the applicable source, directory, manifest, path-safety, staging, rollback, and target-baseline checks while holding the lock. Install an absent target only with atomic no-replace semantics; exchange a present target while preserving and validating the displaced baseline before discarding it. Stop if these semantics are unavailable. After successful publication, verify the canonical target equals the staged identity. Verify every already-published target still matches its staged identity and every remaining target still matches its original baseline before the next publication. If any check fails, stop and restore only this operation's published targets with the same guarded compare-and-swap procedure. Leave targets changed after publication untouched and preserve both versions for manual reconciliation.
6. Immediately before publishing the proposed manifest, repeat the same checks against all applicable sources, directory snapshots, canonical targets, staging files, rollback copies, and the original manifest baseline. Publish the manifest last with the same locked compare-and-swap procedure from step 5, then verify its canonical identity. Use the normative shape in `repository-contract.md`:
...
Completion condition: every integrated material claim is traceable to the immutable regular-file source version that supports it, no source symlink was followed, every processed source still matches its reconciled existence and identity state at finalization, the complete proposal passed validation before canonical publication, locked compare-and-swap publication prevented partial writes and lost concurrent updates, failed publication did not leave known invalid canonical output, prior claims from revised or missing sources are reconciled, unchanged sources were skipped, in-place source changes were rejected, contradictions remain visible, the manifest reflects the operation, and the index can find every changed canonical page.
