...
3. Inventory the original sources needed to support the answer. For every registered source, reject unsafe paths using the shared symlink and canonical-root rules, require an existing regular file, and compare its unfiltered identity with the manifest. For discovery-workspace transcript evidence, also verify the meeting manifest, speaker mapping, discovery-use consent, and direct-quotation consent before using or quoting it; treat `unknown` as not granted. Immediately before returning or saving the answer, recheck every source and consent manifest actually cited, including its regular-file type. If verification fails, identify the affected claims and provenance gap; do not present the stale wiki claim as supported by that source.
4. Answer with inline citations to verified original sources, not citations only to generated wiki pages.
5. Separate requests, evidence, interpretations, assumptions, problem hypotheses, proposals, decisions, contradictions, and unknowns when relevant. Preserve evidence limitations and a request's original wording and requested solution before discussing any reframing. For every included assumption, preserve its owner, consequence if wrong, risk, and proposed validation, using `Unknown` for unavailable values. Keep every problem hypothesis falsifiable with supporting and contradicting evidence listed separately. Do not present source statements as independently verified facts or label them as confirmed findings.
