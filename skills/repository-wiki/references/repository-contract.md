...
6. Preserve disagreements and superseded decisions; never rewrite history to make it look consistent.
7. Update an existing canonical page instead of creating a duplicate summary.
8. Keep `current-state.md` concise. Move history and detail into the appropriate canonical page.
9. Store repository-relative paths in the manifest and reports. In Markdown, calculate each link destination relative to the file containing the link.
10. Show a change plan before touching more than five wiki files, resolving a material contradiction, or making a destructive change.
11. Use Git diffs for review. Do not commit, push, or discard user changes unless explicitly requested.
12. Do not read secrets, credentials, ignored files, or unrelated repository content merely because it is accessible.
