...
const COMMENT_ICON = `<svg class="icon icon-message-square-plus" viewBox="0 0 24 24" aria-hidden="true"><path d="M21 15a4 4 0 0 1-4 4H8l-5 3V7a4 4 0 0 1 4-4h10a4 4 0 0 1 4 4v8Z"/><path d="M12 7v6M9 10h6"/></svg>`;
...
<body data-workspace-id="${e(d.id)}" data-page="${e(active)}">
  ...
  ${nav(active)}
  <main><button class="page-comment review-trigger" type="button" data-page-target="${e(active)}" aria-label="Comment on this page">${COMMENT_ICON}<span>Comment on page</span></button>${body}</main>
...
const CSS = String.raw`:root { --theme-bg: #f5f5f4; --theme-surface: #ffffff; --theme-text: #18181b; --theme-muted: #71717a; --theme-border: #d4d4d8; --theme-accent: #27272a; --theme-accent-soft: #e4e4e7; --theme-warning: #52525b; --paper: var(--theme-bg); --surface: var(--theme-surface); --ink: var(--theme-text); --muted: var(--theme-muted); --line: var(--theme-border); --accent: var(--theme-accent); --accent-soft: var(--theme-accent-soft); --warning: var(--theme-warning); --serif: Georgia, "Times New Roman", serif; --sans: ui-sans-serif, system-ui, sans-serif; }
...
.status { background: var(--accent-soft); border-color: transparent; color: var(--ink); }
...
.decision-banner { background: var(--surface); color: var(--ink); border: 1px solid var(--line); padding: clamp(28px, 5vw, 58px); margin-bottom: 24px; }
.decision-banner .eyebrow { color: var(--muted); }
.decision-banner h2 { max-width: 800px; font-size: clamp(2rem, 5vw, 4.6rem); letter-spacing: -.045em; margin: 14px 0 18px; }
.decision-banner p { max-width: 700px; color: var(--muted); font-size: 1.08rem; }
.decision-action { display: inline-block; margin-top: 8px; color: var(--ink); font-weight: 750; text-underline-offset: 4px; }
...
.policy { border: 1px solid var(--line); background: var(--accent-soft); color: var(--ink); padding: 16px 18px; margin-bottom: 24px; }
...
button { font: inherit; }
.review-trigger { display: inline-flex; min-width: 36px; min-height: 36px; align-items: center; justify-content: center; gap: 8px; border: 1px solid var(--line); border-radius: 9px; background: var(--surface); color: var(--accent); cursor: pointer; padding: 8px; box-shadow: 0 1px 2px #18181b0a; transition: border-color .15s ease, background .15s ease, box-shadow .15s ease, transform .15s ease; }
.review-trigger:hover { border-color: #a1a1aa; background: var(--accent-soft); box-shadow: 0 3px 10px #18181b12; }
.review-trigger:active { transform: translateY(1px); box-shadow: none; }
.review-trigger:focus-visible, #review-drawer button:focus-visible, #review-drawer textarea:focus-visible, #review-drawer input:focus-visible { outline: 3px solid #a1a1aa; outline-offset: 2px; }
[data-record-id][data-field] { position: relative; }
[data-record-id][data-field] > .review-trigger { position: absolute; right: 4px; top: 4px; }
.page-comment { float: right; margin-bottom: 12px; padding-inline: 12px; font-weight: 700; } #review-drawer { position: fixed; left: 50%; top: 50%; width: min(560px, calc(100vw - 32px)); margin: 0; transform: translate(-50%, -50%); max-height: min(720px, calc(100vh - 32px)); border: 1px solid var(--line); border-radius: 14px; padding: 28px; box-shadow: 0 24px 80px #18181b30; }
#review-drawer::backdrop { background: #18181b80; backdrop-filter: blur(2px); } .drawer-close { position: absolute; right: 20px; top: 20px; text-align: right; } .drawer-close button { width: 40px; height: 40px; border: 0; border-radius: 9px; background: transparent; color: var(--muted); cursor: pointer; font-size: 1.5rem; } .drawer-close button:hover { background: var(--paper); color: var(--ink); } #review-form label, #review-form textarea { display: block; width: 100%; } #review-form textarea { min-height: 110px; margin: 8px 0 12px; padding: 12px; border: 1px solid var(--line); border-radius: 9px; resize: vertical; } #review-drawer button[type="submit"], #review-drawer button[data-agent], #review-drawer button[data-resolve], #review-drawer button[data-apply] { min-height: 40px; border: 1px solid var(--accent); border-radius: 9px; background: var(--accent); color: white; cursor: pointer; font-weight: 700; padding: 9px 14px; } #review-drawer button[data-resolve] { margin-left: 8px; border-color: var(--line); background: var(--surface); color: var(--ink); } #review-drawer button:hover { filter: brightness(.96); } .live-thread { border-top: 1px solid var(--line); padding: 14px 0; } .live-thread .tag { display: inline-block; } .proposal { white-space: pre-wrap; background: var(--paper); padding: 10px; overflow-wrap: anywhere; }
...
const JS = String.raw`(() => {
  ...
  document.querySelectorAll('[data-record-id][data-field]').forEach((host) => { if (host.querySelector(':scope > .review-trigger')) return; const button = document.createElement('button'); button.type='button'; button.className='review-trigger'; button.setAttribute('aria-label', 'Comment on ' + host.dataset.recordId + ' ' + host.dataset.field); button.innerHTML=commentIcon; host.append(button); });
